import Node, { type polygonIdentifier } from "./Node"
...
export default class TriNode extends Node{
  override type:polygonIdentifier=3
  constructor(x:number,y:number,parent?:Node){
    super(x,y,parent)
  }

  override draw(ctx:OffscreenCanvasRenderingContext2D,blockSize:number){
  ...
  override addChildren=(...node:Node[])=>this.children.push(...node)

  override getTouchingNodes(nodes:Map<nodeHash,Node>,blockSize:number){
    return [
  ...
  override getViableNodes(nodes:Map<nodeHash,Node>,blockSize:number){
  ...
  override drawLineTo(node:Node,ctx:OffscreenCanvasRenderingContext2D){
    ctx.beginPath()
